...
from src1.darknet53 import MainNet
import matplotlib.pyplot as plt
from tool import utils
import argparse
import configparser
from PIL import Image, ImageDraw, ImageFont
# from detect.detect01 import Detector
from src1.detector_3 import Detector
import cv2
from src1 import cfg

# from baseclass.train import Trainer
#
'''
修改损失函数和激活函数
'''
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
CFGFILE = "..\src1\cfg.ini"
...
class Yolov3Trainer:
    def __init__(self, net, netfile_name, cfgfile=None):
        ...
        # 损失函数定义
        self.conf_loss_fn = nn.BCELoss()  # 定义置信度损失函数
        self.center_loss_fn = nn.BCELoss()  # 定义中心点损失函数
    ...
    def mysigmoid(self, x: torch.Tensor, a=10, b=0.4):
        # x[:] = 1/(1+torch.exp(-a*(x-b)))
        return 1 / (1 + torch.exp(-a * (x - b)))

    def mysoftmax(self, x: torch.Tensor, a=2):
        # x[:] = torch.exp(a*x)/torch.sum(torch.exp(a*x),dim=1,keepdim=True)
        return torch.exp(a * x) / torch.sum(torch.exp(a * x), dim=1, keepdim=True)
    ...
    def _loss_fn(self, output, target, alpha):
        ...
        loss_obj_conf = self.conf_loss_fn(self.mysigmoid(output_obj[:, 0]), target_obj[:, 0])
        loss_obj_center = self.center_loss_fn(self.mysigmoid(output_obj[:, 1:3]), target_obj[:, 1:3])
        loss_obj_wh = self.wh_loss_fn(output_obj[:, 3:5], target_obj[:, 3:5])
        loss_obj_cls = self.cls_loss_fn(self.mysoftmax(output_obj[:, 5:]), target_obj[:, 5].long())
        ...
        loss_noobj = self.conf_loss_fn(self.mysigmoid(output_noobj[:, 0]), target_noobj[:, 0])
    ...
    def logging(self, result, dataloader_len, RECORDPOINT):
        ...
        with open(self.logfile, "r+") as f:
            ...
            if f.readline() == "":
                batchcount = 0
                f.seek(0, 0)
                print("%.2f %d        " % (0.00, 0), end='\r', file=f)  # 要留出足够的空间
    ...
    def train(self):
        ...
        for i in range(self.args.epoch):
            ...
            for j, (target13, target26, target52, img_data) in enumerate(train_loader):
                ...
                if ALPHA < 0.5:
                    self.alpha = torch.randint(6, 10, size=(1,)).float().item() / 10

                loss_13 = self._loss_fn(output_13, target13, alpha=self.alpha)
                loss_26 = self._loss_fn(output_26, target26, alpha=self.alpha)
                loss_52 = self._loss_fn(output_52, target52, alpha=self.alpha)
